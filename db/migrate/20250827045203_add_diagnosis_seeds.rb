class AddDiagnosisSeeds < ActiveRecord::Migration[7.2]
  def up
    return unless Question.count == 0
    ...
    Rails.logger.info "診断用seedデータを作成しました"
  end
  def down
    # 必要に応じてロールバック処理
    Rails.logger.info "診断用seedデータを削除します"
    Choice.destroy_all
    Question.destroy_all
    Diagnosis.destroy_all
    ChoicesDiagnosis.destroy_all
  end
end
