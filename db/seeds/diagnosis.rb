# 質問を作成
    ...
    # 診断結果を作成
    d1 = Diagnosis.find_or_create_by!(title: "ずっと見てたい…天使のまどろみモード💤", explanation: "「ねむねむしあわせ」
    ...
    d2 = Diagnosis.find_or_create_by!(title: "ちょっと不思議でかわいすぎる。考えごとモード💭", explanation: "「今日のごはんは昨日より１グラム多かったな。これは原因を考えねば。」
    ...
    d3 = Diagnosis.find_or_create_by!(title: "吸引力がすごい。甘えんぼモード💓", explanation: "「ちょっとそこに座ってて？…うん、ずっとね。」
      とにかくあなたのそばにいたい！移動すればすぐ追いかけて、
      ひざのすき間にぴったりフィット。
      しっぽはぽふぽふ、目はうるうる…
      全身からあふれる“そばにいたい”ビームが止まらない。", dog_message: "「ひざ、空いてる？（空いてなくても乗るけど）」")

    d4 = Diagnosis.find_or_create_by!(title: "なんか今日めちゃ元気！ハイテンションモード🐕", explanation: "「わーーー！楽しい！！ねえ見て見て！！」
    ...
    d5 = Diagnosis.find_or_create_by!(title: "未知との出会いにワクワク！冒険モード🌍", explanation: "「この先になにがあるのかな？クンクン…新しい発見のにおい！」
