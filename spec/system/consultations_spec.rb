...
RSpec.describe "Consultations", type: :system do
  ...
  context "ログインユーザー" do
    ...
    context "投稿編集、削除" do
      let!(:own_consultation) { create(:consultation, user: user) }
      let!(:other_consultation) { create(:consultation) }
      it "投稿者は編集できる" do
        visit consultation_path(own_consultation)
        click_link "編集"
      ...
      it "投稿者は削除できる" do
        ...
        expect(page).to have_content "お悩み相談を削除しました"
        expect(Consultation.exists?(own_consultation.id)).to be_falsey
      end
    ...
    context "コメント機能" do
      it "コメントが投稿できる", js: true do
        ...
        click_button "投稿"
        expect(page).to have_text "大変ですね", wait: 5
      end
      it "空コメントは投稿できない" do
        visit consultation_path(consultation)
        fill_in "コメント", with: ""
